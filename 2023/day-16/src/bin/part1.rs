...
const START: IVec2 = IVec2::from_array([0, 0]);
...
fn solve(input: &str) -> usize {
    let (_x_max, _y_max, mut map): MapInfo = parse_input(input);

    energize(&mut map, START, RIGHT);

    map.values().filter(|t| t.energized()).count()
}
